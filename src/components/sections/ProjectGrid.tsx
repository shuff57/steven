      ...
      <div id="section-tools" className="mb-20">
        ...
        <div className="grid grid-cols-1 max-w-2xl mx-auto gap-6 w-full">
            {tools.map((project, idx) => {
              ...
              return (
                <ScrollReveal key={project.id} animation="slide-up" delay={idx * 0.05}>
                            ...
                            <div className="flex items-center gap-2 pt-2 border-t border-[var(--color-border)] shrink-0">
                              ...
                              {project.iframeUrl && (
                                <button
                                  ...
                                  aria-label={isExpanded ? `Collapse ${project.title} preview` : `Preview ${project.title}`}
                                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded border transition-colors duration-200 cursor-pointer hover:bg-white/5 ml-auto"
                                  style={{ borderColor: isExpanded ? 'var(--color-accent)' : 'var(--color-border)', color: isExpanded ? 'var(--color-accent)' : 'var(--color-text-secondary)' }}
                                >
                                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                                    {isExpanded
                  ...
                  {project.iframeUrl && (
                    ...
                    >
                      <div
                        ...
                        style={{ borderColor: 'rgba(240,192,96,0.35)' }}
                      >
                        {/* Toolbar */}
                        <div
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-xs text-[var(--color-text-muted)] hover:text-[var(--color-accent)] transition-colors duration-150"
                            >
                              Open ↗
                            </a>
                            <button
                              ...
                              className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors duration-150 text-lg leading-none cursor-pointer border-none bg-transparent"
                            >
                              ×
                            </button>
                          </div>
                        </div>
                        {/* iframe — only mount when expanded so it doesn't load until needed */}
                        {isExpanded && (
                          <iframe
